...
public class InputGenerator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int n : concat(small, medium, large, extraLarge)) {
            ...
            List<Edge> edges = generateConnectedEdges(nodes, rand);
        ...
        System.out.println("input.json сгенерирован успешно!");
    }
    ...
    private static List<Edge> generateConnectedEdges(List<String> nodes, Random rand) {
        ...
        int extraEdges = Math.max(1, nodes.size() / 4);
        for (int i = 0; i < extraEdges; i++) {
            String from = nodes.get(rand.nextInt(nodes.size()));
            String to = nodes.get(rand.nextInt(nodes.size()));
